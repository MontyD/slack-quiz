    ...
    var updateUser = function(reqUsername, answer) {
      user.findOne({ 'username': reqUsername }, 'username', function(err, data) {
        ...
        if (data && data.username !== 'undefined' && data.username !== '') {
          if (answer) {
            user.update({ 'username': data.username }, { $inc: { 'correct': 1 } }).exec();
          } else {
            user.update({ 'username': data.username }, { $inc: { 'incorrect': 1 } }).exec();
          }
        } else {
          var newUser = user({ username: reqUsername });
          newUser.save(function(err) {
            if (err) console.log(err);
            if (answer) {
              user.update({ 'username': reqUsername }, { $inc: { 'correct': 1 } }).exec();
            } else {
              user.update({ 'username': reqUsername }, { $inc: { 'incorrect': 1 } }).exec();
            }
          });
        }
      });
